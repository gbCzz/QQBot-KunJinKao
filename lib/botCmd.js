...
function notice(data) {
	...
	try {
		...
		msgList = data.message.slice(2, data.message.lenth);
		if (!qq || !msgList) throw new Error();
	} catch (error) {
		throw new Error(errors.errName.paraList);
	}
...
function checkNotice(data) {
	...
	if (
		botData?.group?.notice?.[data.group_id]?.[data.sender.user_id]?.[0] == undefined
	)
		return;
	let message = [segment.at(data.sender.user_id, data.sender.nickname), ',\n'];
	if(botData?.group?.notice?.[data.group_id]?.[data.sender.user_id] == undefined)
		return;
	botData.group.notice[data.group_id][data.sender.user_id].forEach(
		(element) => {
			let msgList = [
				`${element.senderName}(${element.senderId}) 于 ${element.time} 提醒您:`,
			];
			msgList.push(...element.message);
			msgList.push('\n');
			message.push(...msgList);
		}
	);
	//清空内存和硬盘中已读的留言
