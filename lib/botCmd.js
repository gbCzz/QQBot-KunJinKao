...
function turnOn(data) {
	try {
		if (
			!botData.member.admin ||
			botData.member.admin.hasOwnProperty(data.sender.user_id) ==
				false ||
			botData.member.admin[data.sender.user_id] == 0
		)
			return '您无权进行此操作';
		else {
			setGroupData(
...
function getJrrp(data) {
	try {
		...
		if (
			...
			botData.member.lastJrrp.hasOwnProperty(data.sender.user_id) ==
				false ||
