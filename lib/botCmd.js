...
let koinData = botData.getMemberData('koin.json');
...
function getJrrp(data) {
	try {
		...
		if (
			...
			if (koinData[data.sender.user_id] == undefined)
				koinData[data.sender.user_id] = 0;
			koinData[data.sender.user_id] += Math.floor(newJrrp * 0.43 * 100) / 100;
		} else {
		...
		botData.setMemberData(
			...
			koinData,
			data.sender.user_id,
			koinData[data.sender.user_id]
		);
		return `${data.sender.nickname} 的今日人品是：${newJrrp}，已折算为 ${
			Math.floor(newJrrp * 0.43 * 100) / 100
		} 点 koin 追加至您的账户，您当前拥有 ${koinData[data.sender.user_id]} 点 koin`;
	} catch (error) {
		return msgT.errFeedback(error);
	}
}
...
async function getHitokoto() {
	try {
		let link = 'https://v1.hitokoto.cn', res;
...
async function getPornography(data) {
	...
	if (koinData.hasOwnProperty(data.sender.user_id) == false || koinData[data.sender.user_id] < 7.00) {
		return {
			status: proxied.status,
			statusText: proxied.statusText,
	...
	koinData[data.sender.user_id] -= 7.00;
	botData.setMemberData(
		'koin.json',
		koinData,
		data.sender.user_id,
		Math.floor(koinData[data.sender.user_id])
	);
	return {
		status: proxied.status,
		statusText: proxied.statusText,
