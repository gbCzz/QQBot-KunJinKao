...
const memberDataPath = path.normalize('./data/memberData/');
const groupDataPath = path.normalize('./data/groupData/');
...
function getGroupData(fileName) {
	...
	try {
		...
		if (fs.existsSync(dataPath) == false)
			fs.mkdirSync(dataPath)
		if (fs.existsSync(groupDataPath) == false)
			fs.mkdirSync(groupDataPath)
