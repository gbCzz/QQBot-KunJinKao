...
function setNotice(data) {
	...
	console.log(noticeData[data.group_id]);
	if (noticeData[data.group_id] == undefined) noticeData[data.group_id] = {};
	...
	return `收到通知，将会在他下次在群内发言的时候提醒他`;
}

function checkNotice(data) {
	if (
		noticeData.hasOwnProperty(data.group_id) == 0 ||
		noticeData[data.group_id].hasOwnProperty(data.sender.user_id) == 0 ||
		noticeData[data.group_id][data.sender.user_id] == 0
	)
		return;
	let message = `[CQ:at,qq=${data.sender.user_id},text=@${data.sender.nickname}],\n`;
