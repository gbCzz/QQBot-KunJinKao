 ...
 * @param {string} path 将要检查的路径
 * @returns 表示是否存在文件的布尔值
 */
function checkConfigFile(path) {
	try {
 ...
 * @param {string} path config.ini 的路径
 * @returns 转换后的对象
 */
function readLoginConfigSync(path) {
	let loginConfig = {};
	try {
		loginConfig = ini.parse(fs.readFileSync(path).toString('utf-8'));
		return loginConfig;
	} catch (error) {
		throw error;
	}
...
export default { readLoginConfigSync, createLoginConfig, checkConfigFile };
