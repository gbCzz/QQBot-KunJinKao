...
import { memberDataPath, groupDataPath } from './botData.js';
...
const loginPrompList = [
	...
	{
		type: 'list',
		message: '请选择 bot 是否会忽略相应自身账号的发言，使用方向键：',
		name: 'ignore_self',
		choices: ['0: 否', '1: 是'],
		default: '0: 否',
	},
...
let dataPath = [
	path.join(groupDataPath, 'notice.json'),
	path.join(groupDataPath, 'onoff.json'),
	path.join(memberDataPath, 'admin.json'),
	path.join(memberDataPath, 'jrrp.json'),
	path.join(memberDataPath, 'lastJrrp.json'),
	path.join(memberDataPath, 'koin.json'),
];
 ...
 */
function checkFileExsist(filepath) {
	try {
...
async function createLoginConfig() {
	...
	try {
		...
		loginConfig.reconn_interval = loginConfig.reconn_interval[0];
